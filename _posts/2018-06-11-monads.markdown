...
> All code produces values, and those values fall into one of two categories: _pure_ or _effect_.
...
Instead of side-effects, think about `Try` values. If it's possible for a computation to fail and return an error, then every value derived from it must also be possible to fail (and therefore also be a `Try`).
...
While this is correct, it's not _really_ what's desired. We'd like to be able to pass our `Effect` to the `pair` function and have the returned type be `Effect[(T, T)]`.
