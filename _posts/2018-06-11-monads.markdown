...
Now what we need is a way to use the result of our _effect_ value and get an _effect_ returned. Instead of `(Effect[A], Effect[A])` returned, we'd like to get back `Effect[(A, A)]`.
