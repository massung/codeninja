...
More importantly, it's guarantees type safety. The compiler knows when we're working with pure values vs. effect values, and won't let us use a pure function in an effect context!
