...
> All code produces values, and those values fall into one of two categories: _pure_ or _effect_. <sup>*</sup>
...
Instead of side-effects, think about `Try` values. If it's possible for a computation to fail and return an error, then every value derived from it must also be possible to fail (and therefore also be a `Try`). <sup>**</sup>
...
And to help ourselves, let's create a way to "map" a _pure_ function and get an _effect_ function back:
...
We are now be able to map the value of an `Effect` through a _pure_ function and get the result back as a new `Effect`.

```scala
@ val x = Effect(Random.nextInt(100))
x: Effect[Int] = Effect(9)
...
And, those two functions must be coded in such a way as to adhere to these three laws:

1. Right identity: `m.flatMap(inject) ≡ m`
2. Left identity: `inject(x).flatMap(f) ≡ f(x)`
...
// right identity
@ Effect(1).flatMap(Effect.apply)
Effect[Int] = Effect(1)

// left identity
@ Effect(1).flatMap(fmap(inc))
Effect[Int] = Effect(2)
...
<sup>*</sup> *There are many different categories of values (e.g. optional, either, asynchronous, ...), but for our problem we're going to focus only on this particular category...*

<sup>**</sup> *If coming from a language like [Go][go], think about functions that return `error` values. If you just blissfully ignore them it's a bug. You need to either handle them (and do something else that can't fail), or propogate the error, which instantly means your return value is now in the category of "may fail".*
