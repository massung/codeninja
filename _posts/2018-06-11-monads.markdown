...
More importantly, it's guarantees type safety. The compiler knows when we're working with pure values vs. effect values, and won't let us use a pure function with an effect value without first mapping it so we get an effect back!
