...
*An astute reader will notice that our `bind` method has the same type signature as Scala's `flatMap` method. In fact, we could rename it `flatMap` and it would work in `for` comprehensions!*
